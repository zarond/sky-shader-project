...
Shader "Skybox/Procedural+Raymarch" {
	Properties{
...
//		_Noise2("Noise texture 2 ", 2D) = "white" {}
		...
		//              for(int i=0; i<int(kSamples); i++)
				...
				half4 frag(v2f IN) : SV_Target
				{
					...
					//raymarch
				{
					ray = -ray ;//normalize(IN.rayDir.xyz);
					const int N = 90;
					const int Nsh = 6;

					const float steps = 5 * 8;
					const float shadowsteps = steps * 3;
					...
					float2 intersect = (intersection1(ray));
					if (ray.y<=0.0) return half4(col,1.0);
					Vec = (intersect.x > +0) ? ray * intersect.x : ray;
		...
		// Temporal jitter: вещь хорошая но надо бы оптимизировать
					const float Jitter = 2.5 * steps;
					Vec += ray * rand(float3(ray.xy, 1)) * Jitter;
#endif
					...
					for (int j = 0; j < N; ++j) {
						...
						if ((curdensity = SampleDensity(V, cheap)) > 0.01)
						//curdensity = SampleDensity(V, cheap);
						{
							//float3 Vec2 = 0;
							shadowdist = 0;
							...
							//return fixed4(ShadeSH9(float4(ray,1.0)),1.0);
							//return half4(IN.skyColor,1.0);

							V2 = V;
							for (int k = 0; k < Nsh; ++k) {
								shadowdist += SampleDensity(V2, cheap);
								//shadowdist += SampleDensity(V2 + k * shadowsteps * _WorldSpaceLightPos0.xyz , cheap);
								V2 += shadowsteps * _WorldSpaceLightPos0.xyz;
							}
							/*float3 temp[Nsh];
							for (int k = 0; k < Nsh; ++k) {
								temp[k] = V2 + k * shadowsteps * _WorldSpaceLightPos0.xyz;
							}
							for (int k = 0; k < Nsh; ++k) {
								temp[k] = SampleDensity(temp[k], cheap);
							}
							for (int k = 0; k < Nsh; ++k) {
								shadowdist += temp[k];
							}*/
							...
							//if (_WorldSpaceLightPos0.y<=0) absorbedlight = 0;
							//float3 absorbedlight = IN.sunColor * exp(-shadowdist)*(1 - exp(-2 * shadowdist));
							//return fixed4(IN.sunColor,1.0);
							//ambient
							...
							float3 ambient = unity_AmbientSky * (0.5 + 0.6 * saturate((V.y - _Low) / (_High - _Low)));// * max(0.0, 1.0 - 2.0*cloudHeight);
							//float3 ambient = IN.skyColor/10 * (0.5 + 0.6*cloudHeight);// * max(0.0, 1.0 - 2.0*cloudHeight);
							...
							lightenergy += absorbedlight * transmittance * (1-exp(-curdensity)) * steps;//
					...
					color *= HenyeyGreenstein(dot(normalize(_WorldSpaceLightPos0.xyz), ray), 0.2);
#endif

#if defined (_GRADIENTBLEND)
					if (ray.y < 0.01) color = color*(ray.y/0.01) + col* (1 - ray.y / 0.01);

#endif
					return fixed4(color*(1 - transmittance) + transmittance * col, 1);
